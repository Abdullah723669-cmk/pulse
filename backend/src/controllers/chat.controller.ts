...
export const checkCanChat = async (
  userAId: string,
  userBId: string
): Promise<{
  canChat: boolean;
  ...
  try {
    ...
    const formatted = await Promise.all(
      userConversations.map(async (cp) => {
        ...
        if (otherParticipant) {
          chatPermission = await checkCanChat(currentUserId, otherParticipant.userId);
        }
...
export const startOrGetConversation = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const permission = await checkCanChat(currentUserId, targetUserId);
...
export const sendMessage = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (otherParticipant) {
      ...
      const permission = await checkCanChat(currentUserId, otherParticipant.userId);
